<script lang="ts">
    ...
    function openAddTaskChat() {
        taskChatModel = new TaskChatModel(new TaskChatEntity({
            messages: [new MessageModel(new Message({
                role: ChatRole.ASSISTANT,
                content: 'Hello! Please enter the task you would like to work on today as well as the time you plan to allot for it.'
            }))]
        }));
        showChat = true;
    }
...
<div class="h-full w-full flex flex-col px-10">
    ...
    <button class="px-3 py-1.5 bg-blue-base rounded hover:bg-blue-hover text-white h-[40px] w-full sm:w-3/4 md:w-1/2 mb-4 mx-auto"
            on:click={openAddTaskChat}>Add Task
    </button>
